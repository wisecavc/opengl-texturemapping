...
const char *WINDOWTITLE = "OpenGL / GLUT Sample -- Cade Wisecaver";
...
float	freq_mod;				// modifies frequency of salmon wiggle
...
Display( )
{
	...
	*/
	...
	float amp = amp_mod;			// modifies sine wave amplitude for salmon swimming
	float freq = freq_mod; 			// modifies sine wave frequency for salmon swimming
	float speed = speed_mod;		// modifies overall speed of salmon swimming animation
	...
	Salmon.UnUse();	//	turns off salmon shader programs
...
InitGraphics( )
{
	...
	*/
	...
	if (!valid)
	{
		fprintf(stderr, "Salmon shader did not compile.\n");
	}
	else
	{
		fprintf(stderr, "Salmon shader successfully compiled.\n");
	}
	...
	Salmon.SetUniformVariable("uShininess", 0.48f ); // whatever you like from P3
